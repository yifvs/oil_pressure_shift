...
本系统基于 **GE/CFM 原厂 Phase II 监控逻辑**，实现对发动机滑油压力漂移的自动化监测与早期预警。系统通过消除 N2 转速、油温、高度等物理干扰因素，提取纯残差信号，结合 OEM 长短均值偏差指标与变点检测算法，精准识别滑油系统异常漂移趋势。
...
└─────────────────────┘     └──────────────────────────┘     └──────────┬──────────────┘
                                      ...
                                      v                               v                               v
                             ┌──────────────────────────┐  ┌──────────────────────────┐  ┌──────────────────────────┐
                             │ analyze_residual_trend.py│  │detect_coking_blockage.py │  │                          │
                             │  趋势分析 & RUL 预估      │  │  结焦/堵塞物理极限监控    │  │        (可并行执行)       │
...
```python
CONFIG = {
    "csv_folder": "G:\\1505左发滑油压力高",   # 原始数据文件夹路径
...
| 🟡 YELLOW | $|\Delta| \geq 1.5$ 或接近红线 | 观察期，需持续监控 |
